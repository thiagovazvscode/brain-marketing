...
export async function POST(
  ...
  try {
    ...
    const current = await db
      .select({ id: playbookDeliverableQualityCriteria.id })
      .from(playbookDeliverableQualityCriteria)
      .where(eq(playbookDeliverableQualityCriteria.blockId, blockId));
    const currentIds = new Set(current.map((r) => r.id));
    if (body.orderedIds.length !== currentIds.size || !body.orderedIds.every((criterionId) => currentIds.has(criterionId))) {
      return NextResponse.json({ error: "A lista não corresponde aos critérios deste bloco." }, { status: 400 });
    }

    await Promise.all(
      body.orderedIds.map((criterionId, index) =>
        db.update(playbookDeliverableQualityCriteria).set({ position: index }).where(eq(playbookDeliverableQualityCriteria.id, criterionId))
      )
    );
